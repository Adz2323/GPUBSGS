...
int main(int argc, char **argv)
{
    ...
    parser.add("-b", "--bsgs", false);
    parser.add("", "--bsgs-size", true);
    ...
    for (unsigned int i = 0; i < args.size(); i++)
    {
        ...
        try
        {
            ...
            else if (optArg.equals("-b", "--bsgs"))
            {
                useBSGS = true;
            }
            else if (optArg.equals("", "--bsgs-size"))
            {
                bsgsSize = std::stoi(optArg.arg);
            }
            ...
            else if (optArg.equals("-f", "--fullyrandom"))
            {
                fullyRandom = true;
            }
        }
    ...
    if (useBSGS && bsgsSize == 0)
    {
        // If BSGS is used but no size is specified, use square root of range
        Int range(&rangeEnd);
        range.Sub(&rangeStart);
        bsgsSize = (int)sqrt((double)range.GetLength());
    }

    if (useBSGS && searchMode != SEARCH_MODE_SA && searchMode != SEARCH_MODE_SX)
    {
        printf("Error: BSGS mode is only compatible with single address (-m ADDRESS) or single xpoint (-m XPOINT) search modes\n");
        exit(-1);
    }
    ...
    if (SetConsoleCtrlHandler(CtrlHandler, TRUE))
    {
        ...
        if (useBSGS)
        {
            v->SearchBSGS(nbCPUThread, gpuId, gridSize, should_exit);
        }
        else
        {
