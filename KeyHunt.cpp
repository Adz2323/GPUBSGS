...
KeyHunt::KeyHunt(const std::string &inputFile, int compMode, int searchMode, int coinType, bool useGpu,
                 ...
                 bool fullyRandom, int bsgsMode, int bsgsSize)
{
    ...
    this->bsgsMode = bsgsMode;
    this->bsgsSize = bsgsSize;
    this->gpuEngineBSGS = nullptr;
...
void KeyHunt::SearchBSGS(int nbThread, std::vector<int> gpuId, std::vector<int> gridSize, bool &should_exit)
{
    double t0 = Timer::get_tick();

    InitializeBSGS(nbThread, gpuId, gridSize);

    // Compute baby steps
    ComputeBabySteps();

    // Prepare for giant steps
    ...
    // Launch giant step threads
    for (int i = 0; i < nbThread; i++)
    {
        params[i].obj = this;
        params[i].threadId = i;
        params[i].isRunning = true;
        params[i].startKey.Set(&rangeStart);
        params[i].startKey.Add((uint64_t)i * bsSize);

        if (useGpu && gpuId.size() > 0)
        {
            thHandles[i] = LaunchThread(_FindKeyGPU, params + i);
        }
        else
        {
            thHandles[i] = LaunchThread(_FindKeyBSGS, params + i);
        }
    }
    ...
    free(params);
    free(thHandles);
    delete gpuEngineBSGS;
    gpuEngineBSGS = nullptr;

    double t1 = Timer::get_tick();
    ::printf("\nBSGS Search completed: Total time %s \n", GetTimeStr(t1 - t0).c_str());
}

void KeyHunt::InitializeBSGS(int nbThread, std::vector<int> gpuId, std::vector<int> gridSize)
{
    if (useGpu && gpuId.size() > 0)
    {
        gpuEngineBSGS = new GPUEngineBSGS(gpuId[0], bsgsSize);
        ::printf("Initializing GPU BSGS engine on device %d\n", gpuId[0]);
    }
    else
    {
        ::printf("Initializing CPU BSGS engine\n");
    }

    // Initialize BSGS parameters
    bsSize = bsgsSize;
    endOfSearch = false;
    ...
    // Initialize other BSGS-specific data structures
    keysToSearch.clear();
    // Add the target public key to keysToSearch
    keysToSearch.push_back(secp->ComputePublicKey(&rangeStart));

    ::printf("BSGS Parameters:\n");
    ::printf("Baby Step Size: 0x%" PRIx64 " (2^%.2f)\n", bsSize, log2((double)bsSize));
    ::printf("Start: %s\n", rangeStart.GetBase16().c_str());
    ::printf("Stop : %s\n", rangeEnd.GetBase16().c_str());
    ::printf("Keys : %d\n", (int)keysToSearch.size());
}

void KeyHunt::ComputeBabySteps()
{
    ::printf("Computing Baby Steps...\n");

    if (gpuEngineBSGS)
    {
        gpuEngineBSGS->generateBabySteps();
        gpuEngineBSGS->sortBabySteps();
    }
    else
    {
        // CPU implementation of baby steps
        Int key;
        Point p;
        for (uint64_t i = 0; i < bsSize; i++)
        {
            key.Set(&rangeStart);
            key.Add((uint64_t)i);
            p = secp->ComputePublicKey(&key);
            hashTable.Add(&p.x, i);
        }
        ::printf("Sorting Baby Steps table...\n");
    ...
    ::printf("Baby Steps computation completed\n");
    hashTable.PrintInfo();
}

void KeyHunt::ComputeGiantSteps()
{
    ::printf("Computing Giant Steps and searching for matches...\n");

    if (gpuEngineBSGS)
    {
        uint64_t target[4];
        // Set up target (public key to search for)
        Point targetPoint = keysToSearch[0];
        targetPoint.x.Get32Bytes((unsigned char *)target);

        uint64_t result[2];
        if (gpuEngineBSGS->solve(target, result))
        {
            // Process the result
            Int privateKey;
            privateKey.SetInt64(result[0]);
            privateKey.Mult((uint64_t)bsSize);
            privateKey.Add(result[1]);
            privateKey.Add(&rangeStart);

            ::printf("\nKey found!\n");
            ::printf("Private Key: 0x%s\n", privateKey.GetBase16().c_str());
            Point publicKey = secp->ComputePublicKey(&privateKey);
            ::printf("Public Key: 0x%s\n", secp->GetPublicKeyHex(true, publicKey).c_str());
    ...
    {
        // CPU implementation of giant steps
        Int giantStep;
        giantStep.Set(&rangeStart);
        Int bs(bsSize);
        Point p = secp->ComputePublicKey(&giantStep);
        p = secp->Negation(p);

        Int key;
        std::vector<uint64_t> off;

        while (giantStep.IsLower(&rangeEnd) && !endOfSearch)
        {
            if (hashTable.Get(&p.x, off))
            {
                for (int o = 0; o < off.size(); o++)
                {
                    key.Set(&giantStep);
                    key.Add(off[o]);
                    if (secp->CheckPubKey(secp->ComputePublicKey(&key), keysToSearch[0]))
                    {
                        ::printf("\nKey found!\n");
                        ::printf("Private Key: 0x%s\n", key.GetBase16().c_str());
                        ::printf("Public Key: 0x%s\n", secp->GetPublicKeyHex(true, keysToSearch[0]).c_str());
                        endOfSearch = true;
                        break;
                    }
                }
            ...
            giantStep.Add(&bs);
            p = secp->AddDirect(p, secp->Negation(secp->ComputePublicKey(&bs)));
        }
    }

    ::printf("Giant Steps computation and search completed\n");
}

void KeyHunt::FindKeyBSGS(TH_PARAM *p)
{
    int thId = p->threadId;
    counters[thId] = 0;

    Int giantStep;
    giantStep.Set(&p->startKey);
    Int bs(bsSize);
    Point px = secp->ComputePublicKey(&giantStep);
    px = secp->Negation(px);
    ...
    while (!endOfSearch)
    {
        ...
        giantStep.Add(&bs);
        px = secp->AddDirect(px, secp->Negation(secp->ComputePublicKey(&bs)));
        counters[thId]++;
    }
...
void KeyHunt::FindKeyGPU(TH_PARAM *p)
{
    int thId = p->threadId;
    counters[thId] = 0;
    ...
    while (!endOfSearch)
    {
        // Prepare batch of keys for GPU processing
        std::vector<Point> points;
        for (int i = 0; i < STEP_SIZE && !endOfSearch; i++)
        {
            Point px = secp->ComputePublicKey(&giantStep);
            points.push_back(secp->Negation(px));
            giantStep.Add(&bs);
        }

        // Process batch on GPU
        std::vector<ITEM> found;
        bool ok = gpuEngineBSGS->LaunchKernel(found, points, keysToSearch[0]);

        if (!ok)
        {
            printf("GPU kernel launch failed\n");
            break;
        }
        ...
        for (int i = 0; i < (int)found.size() && !endOfSearch; i++)
        {
            ITEM it = found[i];
            Int key(&p->startKey);
            key.Add((uint64_t)it.thId * bsSize);
            key.Add((uint64_t)it.distIdx);
