...
OBJET = $(addprefix $(OBJDIR)/, \
        ...
        GPU/GPUEngine.o GPU/GPUEngineBSGS.o GmpUtil.o CmdParse.o HashTable.o)
...
OBJET = $(addprefix $(OBJDIR)/, \
        ...
        hash/ripemd160.o hash/sha256.o hash/sha512.o \
        hash/ripemd160_sse.o hash/sha256_sse.o hash/keccak160.o \
        GmpUtil.o CmdParse.o HashTable.o)
...
$(OBJDIR)/GPU/GPUEngineBSGS.o: GPU/GPUEngineBSGS.cu
	$(NVCC) -G -maxrregcount=0 --ptxas-options=-v --compile --compiler-options -fPIC -ccbin $(CXXCUDA) -m64 -g -I$(CUDA)/include -gencode=arch=compute_$(ccap),code=sm_$(ccap) -o $(OBJDIR)/GPU/GPUEngineBSGS.o -c GPU/GPUEngineBSGS.cu
else
$(OBJDIR)/GPU/GPUEngine.o: GPU/GPUEngine.cu
...
$(OBJDIR)/GPU/GPUEngineBSGS.o: GPU/GPUEngineBSGS.cu
	$(NVCC) -maxrregcount=0 --ptxas-options=-v --compile --compiler-options -fPIC -ccbin $(CXXCUDA) -m64 -O2 -I$(CUDA)/include -gencode=arch=compute_$(ccap),code=sm_$(ccap) -o $(OBJDIR)/GPU/GPUEngineBSGS.o -c GPU/GPUEngineBSGS.cu
endif
endif
