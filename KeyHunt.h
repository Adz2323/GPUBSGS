...
public:
    ...
    void SearchBSGS(int nbThread, std::vector<int> gpuId, std::vector<int> gridSize, bool &should_exit);
    void FindKeyBSGS(TH_PARAM *p);
    void FindKeyBSGSGPU(TH_PARAM *p);
...
private:
    ...
    // BSGS specific members
    int bsgsMode;
    int bsgsSize;
    GPUEngineBSGS *gpuEngineBSGS;

    // BSGS helper methods
    void InitializeBSGS(int nbThread, std::vector<int> gpuId, std::vector<int> gridSize);
    void ComputeBabySteps();
    void ComputeGiantSteps();
